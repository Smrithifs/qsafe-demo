...
# Check if mission flow started successfully (with retries)
echo "⏳ Waiting for Mission Flow Controller to be ready..."
for i in {1..10}; do
    if curl -s http://localhost:5003/api/mission/status > /dev/null 2>&1; then
        echo "✅ Mission Flow Controller: READY"
        break
    fi
    if [ $i -eq 10 ]; then
        echo "❌ Failed to start Mission Flow Controller"
        echo "📋 Check logs/mission_flow.log for details"
